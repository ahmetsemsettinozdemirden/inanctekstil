...
export class BamlAsyncClient {
  ...
  async EvaluateRoomImage(
      image: Image,product_name: string,
    ...
    try {
      ...
      const raw = await this.runtime.callFunction(
        "EvaluateRoomImage",
        {
          "image": image,"product_name": product_name
        },
      ...
      return raw.parsed(false) as EvaluationResult
    } catch (error) {
...
class BamlStreamClient {
  ...
  ): BamlStream<partial_types.EvaluationResult, EvaluationResult> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector ? (Array.isArray(options.collector) ? options.collector : [options.collector]) : [];
      ...
      return new BamlStream<partial_types.EvaluationResult, EvaluationResult>(
        raw,
        (a): partial_types.EvaluationResult => a,
