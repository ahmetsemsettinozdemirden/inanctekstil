...
import { toFabricDescription, toRoomDescription, saveJson, loadManifest as loadManifestFile } from "./lib/helpers";
...
interface ManifestBanner {
  /** Which product SKU to use for the base image */
  sku: string;
  /** Which room to use */
  room: string;
  /** Text content */
  ...
  cta: string;
  disclaimer?: string;
  /** Optional logo path relative to input/ */
  logo?: string;
  /** Theme colors */
  bg_color?: string;
  text_color?: string;
...
interface ManifestCollection {
  ...
  name: string;
  /** Mood for AI generation: "outdoor-natural", "indoor-warm", "minimal-modern" */
  mood: string;
  /** SKUs of products in this collection (swatches used as inputs) */
  skus: string[];
  /** Optional room to use as context */
  room?: string;
}
...
async function runLifestyle(
  ...
  const outDir = path.join(OUTPUT_DIR, sku);
  ...
  const fabric = toFabricDescription(product.fabric);
  ...
  const swatchPath = path.join(INPUT_DIR, product.swatch);
...
async function runTexture(product: ManifestProduct) {
  const sku = product.sku;
  const outDir = path.join(OUTPUT_DIR, sku);
  const intDir = path.join(outDir, "intermediate");
  fs.mkdirSync(intDir, { recursive: true });
...
async function runHeroBanner(
  ...
  const product = manifest.products.find((p) => p.sku === bannerDef.sku);
  if (!product) {
    logger.log(`  ERROR: SKU "${bannerDef.sku}" not found`);
    logger.flush();
    return;
  }
  ...
  if (!roomDef) {
    logger.log(`  ERROR: Room "${bannerDef.room}" not found`);
    logger.flush();
    return;
  }
...
async function runCollectionCard(
  ...
  for (const sku of collectionDef.skus) {
    ...
    if (!product) {
      logger.log(`  WARNING: SKU "${sku}" not found, skipping`);
      continue;
    }
    swatchPaths.push(path.join(INPUT_DIR, product.swatch));
  }
  ...
  let prompt: string;
  if (roomUrl && roomDef) {
    const firstProduct = manifest.products.find((p) => p.sku === collectionDef.skus[0]);
    const fabric = firstProduct
      ? toFabricDescription(firstProduct.fabric)
      : toFabricDescription(manifest.products[0].fabric);
    const room = toRoomDescription(roomDef);
...
async function main() {
  ...
  if (bannerId) {
    ...
    await runHeroBanner(bannerId, bannerDef, manifest);
  ...
  if (collectionId) {
    ...
    await runCollectionCard(collectionId, collectionDef, manifest);
  ...
  const product = manifest.products.find((p) => p.sku === sku);
  if (!product) {
    console.error(`SKU "${sku}" not found in manifest.json`);
    console.error(`Available: ${manifest.products.map((p) => p.sku).join(", ")}`);
    process.exit(1);
  }

  const flows = flowFilter ? [flowFilter] : product.flows;
  const rooms = roomFilter ? [roomFilter] : product.rooms;
  ...
  for (const flow of flows) {
    if (flow === "lifestyle") {
      for (const roomId of rooms) {
        ...
        await runLifestyle(product, roomId, roomDef, manifest);
      }
    } else if (flow === "texture") {
      await runTexture(product);
    }
  }

  console.log("\nDone. Output in:", path.join(OUTPUT_DIR, sku!));
}
