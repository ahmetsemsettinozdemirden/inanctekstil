...
export async function renderHeroBanner(opts: HeroBannerOptions): Promise<Buffer> {
  ...
  // 1. Prepare base image — resize and position on right side
  const baseImage = sharp(opts.baseImagePath);
  const imageZoneWidth = Math.round(width * 0.55);
  const resizedBase = await baseImage
    .resize(imageZoneWidth, height, { fit: "cover", position: "center" })
    .png()
    .toBuffer();

  // 2. Create the background canvas with gradient blend
  // Start with solid bg color, then composite the image on the right
  const gradientWidth = 200; // px of gradient blend zone

  // Create a gradient mask for smooth blending
  const gradientSvg = Buffer.from(`
    <svg width="${imageZoneWidth}" height="${height}">
      <defs>
        <linearGradient id="g" x1="0" y1="0" x2="${gradientWidth}" y2="0" gradientUnits="userSpaceOnUse">
          <stop offset="0" stop-color="white" stop-opacity="0"/>
          <stop offset="1" stop-color="white" stop-opacity="1"/>
        </linearGradient>
      </defs>
      <rect width="${imageZoneWidth}" height="${height}" fill="white"/>
      <rect width="${gradientWidth}" height="${height}" fill="url(#g)"/>
    </svg>
  `);

  // Apply gradient mask to base image
  const maskedBase = await sharp(resizedBase)
    .composite([{ input: gradientSvg, blend: "dest-in" }])
    .png()
    .toBuffer();

  // Create canvas: solid bg + masked image on right
